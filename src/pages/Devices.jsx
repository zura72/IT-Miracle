...
export default function Devices() {
  ...
  return (
    <div className={`min-h-screen py-8 transition-colors duration-300 ${bgClass}`}>
      {notif && (
        <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 px-6 py-3 rounded-lg shadow-lg font-semibold transition-opacity duration-300 cursor-pointer ${dark ? 'bg-green-700' : 'bg-green-600'} text-white`}
      ...
      {modal.open && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-50 p-4">
          <div className={`rounded-xl p-6 w-full max-w-3xl shadow-2xl relative max-h-[90vh] overflow-y-auto ${modalBgClass}`}>
            <button onClick={() => { setModal({ open: false, mode: "", data: {} }); resetPhoto(); }} 
              className="absolute right-4 top-4 text-gray-500 hover:text-gray-700 text-xl" type="button">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>

            <h3 className="text-2xl font-bold mb-6">{modal.mode === "edit" ? "Edit" : "Tambah"} Device</h3>
            ...
            <form onSubmit={doCreateOrEdit} className="space-y-6">
              <div>
                ...
                <div className="flex items-center space-x-4">
                  <label className={`flex flex-col items-center justify-center w-32 h-32 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${dark ? 'border-gray-600 hover:border-blue-500' : 'border-gray-300 hover:border-blue-500'}`}>
                  ...
                  {photoPreview ? (
                    <div className="relative">
                      <img src={photoPreview} alt="preview" className="h-32 w-32 object-cover rounded-lg border shadow" />
                      <button type="button" onClick={removePhoto} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 shadow-md hover:bg-red-600">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="Title" defaultValue={modal.data?.Title || ""} className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`} required autoFocus placeholder="Contoh: Laptop Dell XPS 13" />
                </div>

                <div>
                  <label className="block text-sm font-medium mb-1">Tipe Perangkat</label>
                  <input name="Model" defaultValue={modal.data?.Model || ""} className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`} placeholder="PERSONAL COMPUTER (PC)" />
                </div>

                <div>
                  <label className="block text-sm font-medium mb-1">Status</label>
                  <select name="Status" defaultValue={modal.data?.Status || ""} className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`}>
                ...
                <div>
                  ...
                  <select name="Manufacturer" defaultValue={modal.data?.Manufacturer || ""} className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`}>
                ...
                <div>
                  ...
                  <input name="SerialNumber" defaultValue={modal.data?.SerialNumber || ""} className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`} placeholder="Masukkan nomor serial perangkat" />
                </div>

                <div>
                  <label className="block text-sm font-medium mb-1">ID Pengguna</label>
                  <input name="CurrentOwnerLookupId" defaultValue={modal.data?.CurrentOwnerLookupId ? String(modal.data.CurrentOwnerLookupId) : ""} className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`} placeholder="ID user (angka) untuk lookup" />
                </div>

                <div>
                  <label className="block text-sm font-medium mb-1">Departemen</label>
                  <select name="Divisi" defaultValue={modal.data?.Divisi || ""} className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`}>
                ...
                <div className="flex items-center mt-6">
                  <div className="flex items-center h-5">
                    <input name="AntiVirus" type="checkbox" defaultChecked={!!modal.data?.AntiVirus} className="h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                  </div>
              ...
              <div className="flex gap-3 mt-8 justify-end">
                <button type="button" className={`px-5 py-2.5 rounded-lg font-medium hover:bg-gray-300 transition ${dark ? 'bg-gray-600 text-white hover:bg-gray-700' : 'bg-gray-200 text-gray-700'}`} onClick={() => { setModal({ open: false, mode: "", data: {} }); resetPhoto(); }}>
                  Batal
                </button>
                <button type="submit" className="px-6 py-2.5 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700 transition disabled:opacity-60 flex items-center" disabled={loading}>
                  {loading ? (
                    <>
      ...
      <div className="container mx-auto px-4 max-w-screen-xl">
        <div className={`rounded-xl shadow-lg p-4 md:p-6 ${dark ? 'bg-gray-800' : 'bg-white'}`}>
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              <button className={`px-4 py-2.5 rounded-lg font-medium transition flex items-center ${dark ? 'bg-gray-700 text-white hover:bg-gray-600' : 'border border-gray-300 text-gray-700 hover:bg-gray-50'}`} onClick={handlePrintFiltered}>
              ...
              <button className="px-4 py-2.5 rounded-lg bg-indigo-600 text-white font-medium hover:bg-indigo-700 transition flex items-center" onClick={handlePrintAll}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m4 4h6a2 2 0 002-2v-4a2 2 0 00-2-2h-6a2 2 0 00-2 2v4a2 2 0 002 2z" />
                </svg>
                Cetak (Semua)
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6">
            <div className="flex flex-col">
              ...
              <select className={`px-4 py-2 rounded-lg border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`} value={filter.Status} onChange={(e) => setFilter(f => ({ ...f, Status: e.target.value }))}>
            ...
            <div className="flex flex-col">
              ...
              <select className={`px-4 py-2 rounded-lg border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`} value={filter.Model} onChange={(e) => setFilter(f => ({ ...f, Model: e.target.value }))}>
            ...
            <div className="flex flex-col">
              ...
              <select className={`px-4 py-2 rounded-lg border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition ${dark ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`} value={filter.Divisi} onChange={(e) => setFilter(f => ({ ...f, Divisi: e.target.value }))}>
            ...
            <div className="flex items-end gap-3">
              <button className="px-4 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium transition flex items-center" onClick={fetchData} disabled={loading}>
                {loading ? (
                  <>
                    <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              ...
              <button className="px-5 py-2.5 rounded-lg bg-green-600 hover:bg-green-700 text-white font-medium transition flex items-center" onClick={handleTambah}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function OldPhotoPreview({ meta, fields }) {
  try {
    ...
    if (fields?.id && obj?.fileName) {
      ...
      return (
        <div className="relative">
          <img src={url} alt="current" className="h-32 w-32 object-cover rounded-lg border shadow" />
